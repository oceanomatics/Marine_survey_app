...
- **Three-voice separation and certainty level selector on Cause Consideration** (spec Section 10) — currently one narrative field. Needs owner's stated cause / third-party findings / surveyor's assessment as separate structured inputs with a certainty level dropdown driving hedging language.
- **MINRES BALDER table format for Available Information** (spec Section 12) — already effectively present as `informationSources` + `documentsOnFile` + `documentsRequested`; needs to display in the corpus table format with a Status column, and needs the Background-link toggle to auto-generate attribution sentences in Section 6.
