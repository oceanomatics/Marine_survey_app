...
- **10 July, interactive session (surveyor present) — worked through the priority backlog, then paused mid-merge at the surveyor's request ("stop, pack up").** Sequence:
  ...
  7. **Session paused here at the surveyor's explicit request ("stop, pack up") mid-way through that migration-rename fix.** Working tree is clean, nothing uncommitted, on `overnight-work-2026-07-08`, nothing pushed. **Next session should resume exactly here:**
     - Rename `docs/migrations/027_org_multi_logo_and_assets_bucket.sql` → `031_...sql` (check `docs/TODO.md`/`docs/legal_clauses.md` for any text references to "migration 027" pointing at this file and update them too).
     - Merge `worktree-agent-a5671812647b85165` (Timeline), renaming its `027_timeline_event_ratings.sql` → `032_...sql` as part of the same pass.
     - Re-run `flutter analyze lib/ test/` + `flutter test` after merging to confirm the same clean baseline holds.
...
- [ ] **Still genuinely open — not attempted:** the "Unassigned" bucket collapsible-when-empty behaviour, and the broader "each subsection sizes itself to its actual cue count" layout principle. Both need the cue count lifted up to (or watched redundantly at) `RepairPeriodScopedCuesScreen`'s level to know when a bucket is empty from outside `ContextCuesPanel` — a real but modest architecture change, not a one-line fix; better done as its own focused pass than folded into today's grab-bag of fixes.
...
- [ ] Shares the periodic background mail-check mechanism with §3.14 — **NOT built** (see §3.14; `inboxMessagesProvider` is the natural seam a poller would refresh)
...
- [ ] Automate import: periodic background mail check + new-email badge on Correspondence, sharing ONE polling/event-source mechanism with §3.5's Inbox — **NOT built.** Design intent for whoever picks this up: a single app-level poller (e.g. a `Timer.periodic` provider gated on app lifecycle + connectivity, refreshing `inboxMessagesProvider`) that drives both the Inbox new-mail badge and a Correspondence new-mail badge; the same tick could also fire the §3.3 photo-upload retry queue. Deferred deliberately — background timers are hard to verify headless and burn Gmail API quota while debugging; better done in a supervised session.
