...
**Engine built 13 July 2026** — content (the actual checklist items) is still pending from Andy, so this is untested against real items, but the mechanism is complete and unit-/widget-tested against synthetic ones.
- [✓] Extend `lib/features/checklist` (currently 100% manually ticked...) so items can tick themselves once the underlying data condition is met — **DONE.** Reuses the existing `checklists.linked_section` column (already used for "tap to navigate to the relevant screen") as the auto-tick rule key too, instead of adding a redundant new column — a checklist item whose `linked_section` matches one of §4.3's `case_completeness.dart` keys (`vessel_particulars`, `occurrence`, `damage_description`, `attendance`, `sign_off`, plus the 5 optional ones) auto-ticks once that condition is met. New `checklists.auto_tick_attempted` (migration 038) makes it a **one-shot nudge**: once the system has acted on an item, it's never re-evaluated — so a surveyor manually un-ticking an auto-ticked item is never immediately re-ticked by the next pass even though the underlying data is still true. `ChecklistNotifier.autoTickIfReady()`, called reactively from `checklist_screen.dart` (post-frame, watching the same live case-data providers Case Home's completeness card does). Auto-ticked items show a small "auto" badge (`checklist_item_tile.dart`) so their provenance is visible.
...
- [ ] **Content pending (8 July 2026):** the checklist has no items populated yet — surveyor to get input from a colleague (Andy) on what the actual checklist items should be. Once those exist, whichever ones map cleanly to a data condition should get a matching `linked_section` value to opt into auto-tick.
- [ ] **Not live-verified** — no real checklist items exist yet to test against; `checklist_screen_test.dart` exercises the mechanism with synthetic items instead (including a real bug caught while writing that test: an early version of the widget-test fake unconditionally reassigned provider state on every `autoTickIfReady()` call, which — combined with the screen re-registering its post-frame callback on every rebuild — caused an infinite rebuild loop, only visible as a `pumpAndSettle` timeout. Fixed in the fake; the production `ChecklistNotifier` was never affected, since it only ever reassigns state when an item actually changes).
