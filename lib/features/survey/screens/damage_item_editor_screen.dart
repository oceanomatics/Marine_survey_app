// lib/features/survey/screens/damage_item_editor_screen.dart
//
// Full-screen damage-item editor (TODO.md §3.8, 8 July 2026 review) —
// replaces the old popup/sheet (add_damage_item_sheet.dart), which
// required going through a menu to reach. Damage Type leads the form
// (row 18 — it's what the register groups/sorts by); Location on Vessel
// is hidden once a machinery item is selected, since machinery selection
// already implies location and the field is then redundant (row 19).
//
// Also handles cue -> damage item promotion (row 17): pass [sourceCue]
// to prefill from a context cue (create-new) or merge it into an
// existing item's description (edit) — either way the cue gets linked
// to the resulting damage item on save, and "Confirmed By"/"Confirmation
// Date" auto-populate from the cue's origin/date when not already set
// (row 20).
...
/// Polymorphic link type for cues promoted onto a damage item — same
/// linked_to_type/linked_to_id mechanism as repair periods, machinery
/// nameplates, and occurrences (docs/context_cue_system_review.md).
const damageItemLinkType = 'damage_item';
...
class DamageItemEditorScreen extends ConsumerStatefulWidget {
  const DamageItemEditorScreen({
    super.key,
  ...
  final Future<DamageItemModel> Function(DamageItemModel) onSave;
  final List<OccurrenceModel> occurrences;
  /// Set when opened via cue promotion (row 17) — prefills fields on
  /// create, merges into the description on edit, and gets linked to the
  /// saved damage item either way.
  final SurveyorNote? sourceCue;

  @override
  ConsumerState<DamageItemEditorScreen> createState() =>
      _DamageItemEditorScreenState();
}

class _DamageItemEditorScreenState extends ConsumerState<DamageItemEditorScreen> {
  ...
  void initState() {
    ...
    if (e != null) {
      ...
      // Merge a promoted cue into an existing item (row 17) — append its
      // content to the description rather than overwrite, and only fill
      // Confirmed By/Date if the item doesn't already have them (row 20).
      if (cue != null) {
        _descriptionCtrl.text = _descriptionCtrl.text.trim().isEmpty
            ? cue.content
            : '${_descriptionCtrl.text.trim()}\n\n${cue.content}';
        if (_confirmedBy.isEmpty) _applyCueConfirmation(cue);
      }
    } else if (cue != null) {
      // Create-new from a promoted cue — prefill description + confirmation.
      _descriptionCtrl.text = cue.content;
      _applyCueConfirmation(cue);
    }
  }

  /// Row 20: derive Confirmed By / Confirmation Date from the cue's origin
  /// and creation date rather than requiring manual entry. Third-party
  /// origin is deliberately left unset — too many distinct professional
  /// roles (class surveyor, OEM engineer, dive contractor...) to guess
  /// correctly, so that case still needs a manual pick.
  void _applyCueConfirmation(SurveyorNote cue) {
    switch (cue.origin) {
      case CueOrigin.surveyor:
        _confirmedBy.add(ConfirmedByRole.undersignedSurveyor);
      case CueOrigin.assuredOwner:
        _confirmedBy.add(ConfirmedByRole.ownersRepresentative);
      case CueOrigin.thirdParty:
      case null:
        break;
    }
    _confirmationDate ??= cue.createdAt;
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final saved = await widget.onSave(item);

      final cue = widget.sourceCue;
      if (cue != null) {
        await ref.read(surveyorNotesProvider(widget.caseId).notifier).editNote(
              cue.id,
              content:           cue.content,
              natureOfContent:   cue.natureOfContent,
              evidentiaryWeight: cue.evidentiaryWeight,
              origin:            cue.origin,
              caseSection:       cue.caseSection,
              priority:          cue.priority,
              linkedToType:      damageItemLinkType,
              linkedToId:        saved.damageId,
            );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: BackAppBar(
        title: Text(isEdit ? 'Edit Damage Item' : 'Add Damage Item',
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // ── Damage Type — leads the form (row 18: it's what the ────
            // register list groups/sorts by, so it leads data entry too) ─
            ...
            if (_selectedMachineryId != null && vesselId != null) ...[
              ...
              if (components.isNotEmpty && !_addingNewComponent)
                _StyledDropdown<String?>(
                  ...
                  items: [
                    const DropdownMenuItem(
                        value: null, child: Text('— Not listed —')),
                  ...
                  onChanged: (v) => setState(() {
                    ...
                    if (v != null) {
                      final comp =
                          components.firstWhere((c) => c.componentId == v);
            ...
            Wrap(
              ...
              children: _conditionSuggestions.map((label) {
                ...
                return GestureDetector(
                  onTap: () => setState(
                      () => _conditionCtrl.text = active ? '' : label),
                  child: Container(
                    padding: const EdgeInsets.symmetric(
