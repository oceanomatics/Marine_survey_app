...
//
// driveFolder is reserved for a future Google Drive integration and is not
// shown in the UI yet. Pass context for localFolder and driveFolder.
...
class PhotoPickerSheet extends StatelessWidget {
  ...
  static Future<List<File>?> resolveFiles({BuildContext? context}) async {
    ...
    if (!dirExists) {
      debugPrint('[FolderImport] path is not a local directory (cloud provider?)');
    ...
    if (imageFiles.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('No images found in:\n$dirPath'),
  ...
  static Future<List<Uint8List>> resolveBytes(
    ...
    switch (source) {
      ...
      case PhotoPickSource.localFolder:
        ...
        try {
          imageFiles = Directory(dirPath)
              ...
              .whereType<File>()
              .where((f) =>
                  _imageExts.contains(p.extension(f.path).toLowerCase()))
              .toList()
            ..sort((a, b) =>
      ...
      // ── Google Drive folder (future — not shown in UI yet) ─────────────────
      ...
      child: Column(
        ...
        children: [
          ...
          _Tile(
            ...
            subtitle:
                'Pick a folder and select all images at once',
            color: accentColor,
            onTap: () => Navigator.pop(context, PhotoPickSource.localFolder),
          ),
        ],
      ),
    ));
